---
name: tanglegram
description: 当输入为两棵 tip 集合一致的系统发育树文件（如 IQ-TREE 的 .treefile）加一张含 label 列的注释表，且用户要求绘制 co-phylogenetic tanglegram（缠绕图）、并按 metadata 列着色连线、标注 bootstrap 支持度或分别调整左右树分支颜色时触发。不负责建树、比对或祖先状态重建。
skill_id: tanglegram
version: 0.9.0
category: analysis
---
...
已有两棵**同一批 tip** 的系统发育树（如两个基因/蛋白家族分别用 IQ-TREE、RAxML 构建的 `.treefile`），希望面对面比较拓扑：镜像右树、同名 tip 连线、连线按某个注释列（如 family、genus、分组）着色、在**两棵树上**标注 bootstrap 支持度、左右树分支各自指定颜色。
...
| `--format` / `--width` / `--height` / `--dpi` | 否 | `pdf`/`png`/`both`（默认 `both`）、图宽 14、图高 10、PNG 分辨率 1000。 |
...
- 镜像与旋转只改绘图坐标不改拓扑；可用 `ape::all.equal.phylo` 校验。
